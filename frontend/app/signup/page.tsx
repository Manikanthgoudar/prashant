  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (data.token) {
          localStorage.setItem('stella-user', JSON.stringify({ id: data.user.id, name: data.user.name, email: data.user.email, token: data.token }))
        ...
        if (data.token) {
          ...
          localStorage.setItem('stella-user', JSON.stringify({ id: data.user?.id, name: formData.name, email: formData.email }))
          window.dispatchEvent(new Event('stella-user-update'))
        }
        setSuccess(true)
      }
